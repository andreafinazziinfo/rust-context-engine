...
*   **Ponytail reduced to a one-line stub for Claude Code / Copilot** — unlike Cursor/Windsurf (which get the full ruleset via a dedicated always-apply `.cursor/rules/ponytail.mdc`), Claude Code and GitHub Copilot have no such rule-file convention; `CLAUDE.md`/`copilot-instructions.md` is the only channel that reaches them, and it only ever got "Always apply the Ponytail philosophy (YAGNI, minimal code, deletion over addition)." The full upstream ruleset ([DietrichGebert/ponytail](https://github.com/DietrichGebert/ponytail): decision ladder, rules, boundaries) is now inlined into the `CLAUDE.md`/`copilot-instructions.md` block instead.
