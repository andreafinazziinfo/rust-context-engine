...
| 7 | `ls` filter's truncation summary (`... and N more entries ...`) silently corrupts counts when piped into `wc -l`/`awk`/other parsers — no error, no signal, a plausible-looking wrong number (36 real entries in a repro directory, `ls -1 \| wc -l` reports 19). Same underlying flaw as #16 (SIGPIPE): filtered output is safe to read, not safe to compose. Issue #77. | Medium — silent data corruption is worse than a crash; affects any script/hook/agent pipeline that counts or parses filtered output | Fixed (`docs/PLAN_RELIABILITY_2026-09.md` REL-1/2/3) | Repo owner |
| 8 | `execute_with_filter`'s `Combined` mode (`cargo test`, `npm`/`yarn`/`pnpm` install, `vitest`) joined stderr+stdout with an unconditional `\n` separator, adding a spurious blank line whenever stderr was already newline-terminated — a deterministic but still-wrong line count for anything piping that output. Found sweeping every wrapped command for the #77 failure class (REL-2). | Low — off-by-one, not a magnitude-wrong count, but same corruption family | Fixed (`join_streams`, same change as #7) | Repo owner |
